...
class DeleteListUseCase @Inject constructor(private val appRepo: AppRepo, schedulerProvider: SchedulerProvider) : CompletableUseCase<ShoppingList>(schedulerProvider) {
    override fun buildUseCaseCompletable(params: ShoppingList?) : Completable {
        checkNotNull(params)
        return appRepo.deleteList(params)
    }
}